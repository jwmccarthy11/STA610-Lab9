...
$$Y_{ij}=X^T_{ij}\beta+b_i + \epsilon_{ij}$$ where $b_i\sim N(0,\sigma_b^2)\perp\epsilon_{ij}\sim N(0,\sigma^2_e)$ and $E(Y_{ij}|b_i)=X^T_{ij}\beta+b_i$. Here $Y_{ij}$ represents the presence of badgers at time $i$ on farm $j$.
...
* Holding other variables constant, in general, for every 1 unit increase in the number of active settlements in the field, we expect the odds of the presence of signs of badgers in the farmyard to increase by 61.1%  ($e^{0.477}3 -1 \approx  61.1%$).
...
```{r}
sigma2hat <- sigma(model2)*sigma(model2)
tau2hat <- as.numeric(VarCorr(model2)$farm_code_numeric)
corrs <- c(sigma2hat,tau2hat,sigma2hat/(sigma2hat+(pi^2/3)))
corrs
```
